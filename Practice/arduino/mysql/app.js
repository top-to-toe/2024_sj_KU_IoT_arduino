...
app.get('/', function (req, res) {
    fs.readFile('list.html', 'utf8', function (error, data) {
        if (error) {
            ...
            mySqlClient.query('SELECT * FROM dev01', function (error, dev01Results) {
                if (error) {
                    console.log('error: ', error.message); // 쿼리 오류
                } else {
                    mySqlClient.query('SELECT * FROM dev02', function (error, dev02Results) {
                        if (error) {
                            console.log('error: ', error.message); // 쿼리 오류
                        } else {
                            res.send(ejs.render(data, { dht: dev01Results, power: dev02Results })); // 조회된 데이터를 ejs 템플릿에 전달
                        }
                    });
                }
            });
